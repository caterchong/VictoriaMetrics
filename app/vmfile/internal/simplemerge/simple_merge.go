...
func SimpleMerge(fromPaths []string, toPath string, flag int) {
	...
	if flag&2 != 0 {
		fs.MustMkdirIfNotExist(filepath.Join(toPath, dataDirname, "big"))
		...
		if flag&2 != 0 {
			...
			for _, partitionName := range []string{"small", "big"} {
				...
				for _, month := range monthly {
					...
					dstMonthlyDir := filepath.Join(toPath, dataDirname, partitionName, month)
			...
			for month := range monthMap {
				fs.MustMkdirIfNotExist(filepath.Join(toPath, dataDirname, "big", month))
			}
		}
	}
}
